...
def main(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        import subprocess
        import sys
        train_script = os.path.join(PROJ_DIR, 'scripts', 'train', 'train_windows.py')
        gpu_str = ",".join([str(gpu) for gpu in args.gpus])
        cmd = [sys.executable, train_script, tmp_config_path, '--gpu', gpu_str]
        
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, check=True, cwd=PROJ_DIR)
            text = result.stdout + result.stderr
        except subprocess.CalledProcessError as e:
            text = e.stdout + e.stderr
            print_log(f"Training failed: {e}")
            print_log(f"Output: {text}")
            continue
    ...
    for metric_name in round_metrics[0]:
        ...
        if hasattr(values[0], 'pvalue'):
            ...
            print(f'{metric_name}: {round(np.mean(values), 4)} ± {round(np.std(values), 4)}')
            print(f'{metric_name}_pvalue: {np.mean(pvalues)} ± {np.std(pvalues)}')
        else:
            print(f'{metric_name}: {round(np.mean(values), 4)} ± {round(np.std(values), 4)}')
    print()
