...
class EnhancedBesselBasis(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # [..., 1]
        ...
        # Compute arguments for sin function
        arguments = self.bessel_weights * x_safe  # [..., num_basis]
        
        # Compute sin(kx) with numerical stability check
        numerator = torch.sin(arguments)  # [..., num_basis]
        ...
        small_distance_mask = x < self.eps
        if small_distance_mask.any():
            # More accurate Taylor expansion: sin(kx)/x ≈ k - k³x²/6
            k = self.bessel_weights  # [num_basis]
            x_expanded = x.expand_as(result)  # [..., num_basis]
            taylor_order1 = self.prefactor * k  # Leading term
            taylor_order2 = -self.prefactor * (k**3 * x_expanded**2) / 6  # Correction term
            taylor_approx = taylor_order1 + taylor_order2
            
            result = torch.where(
                small_distance_mask.expand_as(result),
                taylor_approx,
                result
            )
