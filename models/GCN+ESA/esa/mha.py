...
# Conditional import for xformers
try:
    from xformers.ops import memory_efficient_attention
    XFORMERS_AVAILABLE = True
except ImportError:
    XFORMERS_AVAILABLE = False
    print("[WARNING] xformers not available, falling back to torch attention")
    
    def memory_efficient_attention(*args, **kwargs):
        raise NotImplementedError("xformers not available, use torch attention instead")


class MAB(nn.Module):
    def __init__(self, dim_Q, dim_K, dim_V, num_heads, dropout_p=0.0, xformers_or_torch_attn="torch"):
        super(MAB, self).__init__()

        # Force torch attention if xformers not available
        if not XFORMERS_AVAILABLE and xformers_or_torch_attn == "xformers":
            print(f"[WARNING] xformers requested but not available, using torch attention instead")
    ...
    def forward(self, Q, K, adj_mask=None):
        ...
        if self.xformers_or_torch_attn == "xformers" and XFORMERS_AVAILABLE:
        ...
        else:  # Use torch attention (either requested or fallback)
