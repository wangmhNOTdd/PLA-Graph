...
# -*- coding: utf-8 -*-
"""
Processing script for PDBBind v2020-other-PL dataset
"""
import os
import sys
import json
...
def parse_args():
    parser = argparse.ArgumentParser(description='Process PDBBind v2020-other-PL dataset')
    parser.add_argument('--data_dir', type=str, 
                        default='./datasets/v2020-other-PL',
                        help='Directory containing the v2020-other-PL dataset')
    parser.add_argument('--out_dir', type=str,
                        default='./datasets/v2020-other-PL/processed',
...
def split_dataset(entries, test_ratio, valid_ratio, seed=42):
    """Split dataset into train/valid/test sets"""
    np.random.seed(seed)
    indices = np.random.permutation(len(entries))
    
    n_test = int(len(entries) * test_ratio)
    n_valid = int(len(entries) * valid_ratio)
    n_train = len(entries) - n_test - n_valid
    ...
    print_log(f'Dataset split: Train={len(train_entries)}, Valid={len(valid_entries)}, Test={len(test_entries)}')
...
def main():
    ...
    entries = load_index_file(args.data_dir)
    ...
    # Split dataset
    train_entries, valid_entries, test_entries = split_dataset(
        valid_entries, args.test_ratio, args.valid_ratio
    )
    
    # Process each split
    for split_name, split_entries in [('train', train_entries), ('valid', valid_entries), ('test', test_entries)]:
    ...
    print_log(f'Dataset processing completed. Split info saved to {split_info_file}')
